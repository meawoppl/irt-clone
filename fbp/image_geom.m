...
[st.nx st.dx] = image_geom_down_round(st.nx, st.dx, down);
[st.ny st.dy] = image_geom_down_round(st.ny, st.dy, down);
st.offset_x = st.offset_x / down; % because in "pixel" units
st.offset_y = st.offset_y / down;
st.down = st.down * down;
st.dim = [st.nx st.ny];

if st.is3
	[st.nz st.dz] = image_geom_down_round(st.nz, st.dz, down);
	st.offset_z = st.offset_z / down;
	st.dim = [st.nx st.ny st.nz];
end
...
if st.is3
	...
	elseif any(st.dim * down ~= size(st.mask))
		pr st.dim
		pr st.dim * down
		pr size(st.mask)
