...
st.ns = 4 * ceil(st.ns / down / 4); % multiple of 4 for simd
st.nt = 2 * ceil(st.nt / down / 2); % keep it even
...
if numel(st.orbit_start) > 1
	st.orbit_start = st.orbit_start(1:down:si.na);
	user_specified = true;
end
...
elseif all(diff(si.source_zs) == 0) % axial
	st.na = max(1, round(si.na / down)); % at least one view
	...
	if abs(round(na1) - na1) < tol % integer views per turn
		na2 = round(na1 / down); % new # of views in 1 turn (integer)
